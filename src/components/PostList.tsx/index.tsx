...
const PostList: FC<PostListProps> = ({ category, tag, className }) => {
  ...
  return (
    <div
      className={cn('flex w-full flex-col items-center gap-8 pt-20', className)}
    >
      <h1 className={cn('text-2xl font-medium', !category && !tag && 'hidden')}>
        {category ? category : `#${tag}`}
      </h1>
      <div className="container grid grid-cols-2 gap-x-3 gap-y-6 pb-24 lg:gap-x-7 lg:gap-y-12">
        {postPages?.pages
